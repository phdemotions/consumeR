...
#'     \item \code{eta_squared}: Effect size (\eqn{\eta^2})
...
compare_groups_anova <- function(data,
  ...
  interpretation <- paste0(
    ...
    ", p = ", round(p_value, 4), "). ",
    "The effect size is ", effect_interp, " (eta-squared = ", round(eta_squared, 3), "). "
  )
...
print.anova_result <- function(x, show_assumptions = FALSE, show_posthoc = TRUE, show_publication = FALSE, ...) {
  ...
  cat("EFFECT SIZE:\n")
  cat("  eta-squared =", x$eta_squared, "(", x$effect_interpretation, ")\n")
